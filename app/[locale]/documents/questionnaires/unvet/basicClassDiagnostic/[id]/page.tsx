...
import { getUnvetBcdResult } from '@/utils/reports/unvet/bcd/bcd';
...
export default async function BasicClassDiagnosticPDF({ params }: { params: { id: string } }) {
  ...
  const result = await getUnvetBcdResult(params.id);
  if (!result) return <NoResultsAlert />;

  const barValues = result.results.charts.barChart;
