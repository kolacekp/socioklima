...
import { getSoklRecommendationsResult } from '@/utils/reports/sokl/recommendations/recommendations';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'documents.questionnaires.sokl.recommendations' });
...
export default async function RecommendationsPDF({ params }: { params: { id: string } }) {
  ...
  const result = await getSoklRecommendationsResult(params.id);
