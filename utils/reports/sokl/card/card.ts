...
import { Categories, Count, Factors, Options, SoklPupilCardResult } from './card.model';
...
export const getSoklPupilCardResult = async (
  questionnaireId: string,
  pupilId: string
): Promise<SoklPupilCardResult | null> => {
  ...
  countOptions(results, optionsFromOthers, undefined, 'sokl.questions.8', true, pupilId, comments);
  ...
  const q1 = ownResult.questions.filter((question) => {
    return question.name === 'sokl.questions.1';
  });
  const q2 = ownResult.questions.filter((question) => {
    return question.name === 'sokl.questions.2';
  });
  const q3 = ownResult.questions.filter((question) => {
    return question.name === 'sokl.questions.3';
  });
  const q4 = ownResult.questions.filter((question) => {
    return question.name === 'sokl.questions.4';
  });
  ...
  const result: SoklPupilCardResult = {
    info: {
      pupil: {
