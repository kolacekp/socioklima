...
import { getSoklChoicesSummaryResult } from '@/utils/reports/sokl/choices/choices';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'documents.questionnaires.sokl.choices_summary' });
...
export default async function PupilCardPDF({ params }: { params: { id: string } }) {
  ...
  const result = await getSoklChoicesSummaryResult(params.id);
