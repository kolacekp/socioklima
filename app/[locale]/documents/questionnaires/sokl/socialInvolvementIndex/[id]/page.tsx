...
import { getSoklSocialIndexResult } from '@/utils/reports/sokl/index';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({
    locale,
    namespace: 'documents.questionnaires.sokl.social_involvement_index'
  });
...
export default async function SocialInvolvementIndexPDF({ params }: { params: { id: string } }) {
  ...
  const result = await getSoklSocialIndexResult(params.id);
