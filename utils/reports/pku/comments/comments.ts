...
import { PkuCommentsResult } from './comments.model';
...
export const getPkuCommentsResult = async (questionnaireId: string): Promise<PkuCommentsResult | null> => {
  ...
  const commentsQ1 = countComments(results, 'pku.questions.1');
  const commentsQ2 = countComments(results, 'pku.questions.2');
  const commentsQ3 = countComments(results, 'pku.questions.3');
  const commentsQ4 = countComments(results, 'pku.questions.4');

  const result: PkuCommentsResult = {
    info: {
